...
ll rec(ll pos,ll pre)
{
    ...
    if(pre==0)
    {
        ...
        if( (ar[pos]-h[pos]) >  ar[pos-1])
        {
            ret=max(ret,1+rec(pos+1,1));
        }
       if( (ar[pos]+h[pos]) < ar[pos+1])
        {
            ret=max(ret,1+rec(pos+1,2));
        }
    ...
    {
        if(ar[pos]-h[pos]>h[pos-1]+ar[pos-1])
        {
             ret=max( ret,1+rec(pos+1,1) );
        }
         if( (ar[pos]+h[pos]) <  ar[pos+1])
