...
ll tree[300005];
ll a,b,n;
void insert(ll idx,ll s,ll e,ll i,ll j)
{
    if(s==i  and  e==j)
    {
        tree[idx]++;
    ...
    if(j<=mid)
    {
        insert(idx*2,s,mid,i,j);
    }
    else if(i>mid)
    {
        insert(idx*2+1,mid+1,e,i,j);
    }
    else
    {
        insert(idx*2,s,mid,i,mid);
        insert(idx*2+1,mid+1,e,mid+1,j);
    }
}
ll Q(ll idx,ll s,ll e,ll i)
{
    if(s==e)
    {
        return tree[idx];
    ...
    if(i<=mid)
    {
       return tree[idx]+ Q(idx*2,s,mid,i);
    }
    else
    {
        return tree[idx]+Q(idx*2+1,mid+1,e,i);
    }
}
