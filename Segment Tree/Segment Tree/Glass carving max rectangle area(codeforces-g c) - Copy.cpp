...
void init(LL  ind, LL  st, LL  ed)
{
    ...
    LL  mid = (st + ed)/2,lft = ind * 2,rgt = lft + 1;
    init(lft, st, mid);
    init(rgt, mid + 1, ed);
    if(seg[lft].l) seg[ind].l = seg[lft].l;
    else if(seg[lft].r) seg[ind].l = seg[lft].r;
    else if(seg[rgt].l) seg[ind].l = seg[rgt].l;
    else if(seg[rgt].r) seg[ind].l = seg[rgt].r;

    if(seg[rgt].r)      seg[ind].r = seg[rgt].r;
    else if(seg[rgt].l) seg[ind].r = seg[rgt].l;
    else if(seg[lft].r) seg[ind].r = seg[lft].r;
    else if(seg[lft].l) seg[ind].r = seg[lft].l;
    ...
    {
        LL  v = 0;
        if(seg[lft].r)      v = ed - seg[lft].r;
        else if(seg[lft].l) v = ed - seg[lft].l;
        seg[ind].val = max(seg[ind].val, v);
    }
}
void inserth(LL  ind, LL  st, LL  ed, LL  pos)
{
    ...
    LL  mid = (st + ed)/2,lft = ind * 2,rgt = lft + 1;
    if(pos <= mid) inserth(lft, st, mid, pos);
    else  inserth(rgt, mid + 1, ed, pos);
    ...
    {
        LL  v = 0;
        if(seg[rgt].l)       v = seg[rgt].l - st + 1;
        else if(seg[rgt].r)  v = seg[rgt].r - st + 1;
        seg[ind].val = max(seg[ind].val, v);
    }
    else if(seg[rgt].l == 0 && seg[rgt].r == 0)
    {
        LL  v = 0;
        if(seg[lft].r)       v = ed - seg[lft].r + 1;
        else if(seg[lft].l)  v = ed - seg[lft].l + 1;
        seg[ind].val = max(seg[ind].val, v);
    }
}
