...
void init(ll idx,ll st,ll end)
{
    ...
    tree[idx].Mxx=max(min(tree[L].Mx,tree[R].Mx),max(tree[L].Mxx,tree[R].Mxx));
...
info  Q(ll idx,ll st,ll end,ll i,ll j)
{
    ...
    else
    {
        ...
        cc.Mx=max(aa.Mx,bb.Mx);

        cc.Mxx=max(aa.Mxx,bb.Mxx);
