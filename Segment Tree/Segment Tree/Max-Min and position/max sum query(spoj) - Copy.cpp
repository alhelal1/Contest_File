...
ll n,m,a,b,c,ar[100005];

void init(ll idx,ll st,ll end)
{
    if(st==end)
    {
        tree[idx].sum=ar[st];
    ...
    tree[idx].lsum=max(tree[R].rsum,tree[R].sum+tree[L].rsum);
    tree[idx].ans=max(max(tree[L].ans,tree[R].ans),tree[L].rsum+tree[R].rsum);
}
...
info Q(ll idx,ll st,ll end,ll i,ll j)
{
    ...
    if(j<=mid)
    {
        return Q(L,st,mid,i,j);

    }
    else if(i>mid)
    {
        return Q(R,mid+1,end,i,j);
    }
    else
    {
        ...
        cc.lsum=max(cc.lsum,aa.sum+bb.rsum);

        cc.rsum=max(cc.rsum,bb.sum+aa.lsum);
