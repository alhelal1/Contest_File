...
///int comb[MAX][MAX];void calComb(int n,int m=0){for(int i=0;i<=n;i++){comb[i][0]=1;for(int j=1;j<=i;j++){comb[i][j]=comb[i-1][j]+comb[i-1][j-1];if(m>0)comb[i][j]%=m;}}}
...
void update(ll idx,ll s,ll e,ll i,ll j,ll val)
{
    if(s==i and e==j)
    {
        tree[idx].sum+=(e-s+1)*val;
        tree[idx].lazy+=val;
        return ;
    }
    tree[idx].sum+=(j-i+1)*val;
    ll mid=(s+e)>>1;
    if(j<=mid)
        update(idx*2,s,mid,i,j,val);
    else if(i>mid)
        update(idx*2+1,mid+1,e,i,j,val);
    else
    {
        update(idx*2,s,mid,i,mid,val);
        update(idx*2+1,mid+1,e,mid+1,j,val);
    }
}
ll Q(ll idx,ll s,ll e,ll i,ll j,ll val)
{
    if(s==i and e==j)
    {
        return   tree[idx].sum;
    }
    ll value=tree[idx].lazy*(j-i+1);
    ll mid=(s+e)>>1;
    if(j<=mid)
       return   value + Q(idx*2,s,mid,i,j,val);
    else if(i>mid)
        return  value +  Q(idx*2+1,mid+1,e,i,j,val);
    else
       ll p1= Q(idx*2,s,mid,i,mid,val);ll p2= Q(idx*2+1,mid+1,e,mid+1,j,val);
       return p1+p2+value;
}
int main()
{
        ...
        rep(i,m)
        {
            ...
            if(a==0)
            {
                III(u,v,val);update(1,1,n,u+1,v+1,val);
            }
            else
            {
                II(u,v);ll aa=Q(1,1,n,u+1,v+1,0);
            }
        }
}
