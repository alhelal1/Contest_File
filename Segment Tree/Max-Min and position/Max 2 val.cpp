...
void insert(ll idx,ll s,ll e)
{
    ...
    tree[idx].x=max(tree[idx*2].x,tree[idx*2+1].x);

    tree[idx].y=max(max(tree[idx*2].y,tree[idx*2+1].y),min(tree[idx*2].x,tree[idx*2+1].x));
...
void up(ll idx,ll s,ll e,ll i,ll val)
{
    if(s==i and e==i)
    {
        tree[idx].x=val;
    ...
    up(idx*2,s,mid,i,val);
    else

    up(idx*2+1,mid+1,e,i,val);
...
info Q(ll idx,ll s,ll e,ll i,ll j)
{
    if(s==i and e==j)
    {
        return tree[idx];
    ...
    if(j<=mid)
    {
        return Q(idx*2,s,mid,i,j);
    }
    else if(i>mid)
    {
       return  Q(idx*2+1,mid+1,e,i,j);
    }
    else
    {
        info  p1=Q(idx*2,s,mid,i,mid);

        info  p2=Q(idx*2+1,mid+1,e,mid+1,j);

       info res;

       res.x=max(p1.x,p2.x);

       res.y=max(max(p1.y,p2.y),min(p1.x,p2.x));
