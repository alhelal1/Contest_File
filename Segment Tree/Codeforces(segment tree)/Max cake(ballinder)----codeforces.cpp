...
void up(ll idx,ll st,ll end,ll i,ll val)
{
    ...
    if(i<=mid)
    {
        up(idx*2,st,mid,i,val);
    }
    else  up(idx*2+1,mid+1,end,i,val);
...
ll q(ll idx,ll st,ll end,ll i,ll j)
{
    if(st>end)return 0;
    if(st==i and end==j)
    {
    ...
    if(j<=mid)  return  q(idx*2,st,mid,i,j);
    else if(i>mid)  return q(idx*2+1,mid+1,end,i,j);
    else
    {
        ll aa=q(idx*2,st,mid,i,mid);
        ll bb=q(idx*2+1,mid+1,end,mid+1,j);
...
int main()
{
    ...
    repp(i,n)
    {
        ll x=lower_bound(ar+1,ar+n+1,ar1[i])-ar;
        ll y=q(1,1,n,1,x-1);
        up(1,1,n,x,y+ar1[i]);
    }
    double ans=tree[1];
    ans=ans*double(acos(-1.0));
    cout<<ans<<endl;
