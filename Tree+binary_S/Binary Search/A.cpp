...
//struct info{ll x,y;}arr[10005];bool com(info a,info b){return (a.x<b.x) or (a.x==b.x and a.y<b.y);}

ll ar[200004],ar1[200005];
...
int main()
{
   ...
   //mem(ar1,false);
    repp(i,m)
    {
        ...
        {
            ll lo=j,hi=len(v[i])-1;

            while(lo<=hi)
            {
                ll mid=(lo+hi)>>1;
                if(v[i][mid]-v[i][j]+1-k<=mid-j+1)
                {
                    r=max(r,mid-j+1);
                    lo=mid+1;
                }
